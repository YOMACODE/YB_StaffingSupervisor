...
namespace YB_StaffingSupervisor.DataAccess.Repository
{
    public class AttendanceCorrectionRepository : BaseRepository, IAttendanceCorrectionRepository
    {
        ...
        public async Task<DataTable> GetAttendanceCorrectionExport(string userid, string YomaId, string AttendenceFrom, string AttendenceTo, string status)
        {
            using (var dbconnect = connectionFactory.GetDAL)
            {
                SqlParameter[] sqlparameters =
                ...
                DataTable dataTable = await Task.Run(() => dbconnect.SPExecuteDataTable("[WebApplication_SP].[usp_Supervisor_AttendanceCorrectionRequest_ApproveReject_SelectAll_SelectById]", sqlparameters, "dt"));
